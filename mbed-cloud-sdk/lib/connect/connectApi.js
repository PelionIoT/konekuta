...
var connectedDeviceAdapter_1 = require("./models/connectedDeviceAdapter");
 ...
 */
var ConnectApi = (function (_super) {
    ...
    */
    ConnectApi.prototype.notify = function (notification) {
        var _this = this;
        // Notification can be null
        if (!notification)
            return;
        if (notification["notifications"]) {
            notification["notifications"].forEach(function (notification) {
        ...
        if (notification["registrations"]) {
            notification["registrations"].forEach(function (device) {
        ...
        if (notification["reg-updates"]) {
            notification["reg-updates"].forEach(function (device) {
        ...
        if (notification["de-registrations"]) {
            notification["de-registrations"].forEach(function (deviceId) {
                _this.emit(ConnectApi.EVENT_DEREGISTRATION, deviceId);
            });
        }
        if (notification["registrations-expired"]) {
            notification["registrations-expired"].forEach(function (deviceId) {
                _this.emit(ConnectApi.EVENT_EXPIRED, deviceId);
            });
        }
        if (notification["async-responses"]) {
            notification["async-responses"].forEach(function (response) {
    ...
    ConnectApi.prototype.listConnectedDevices = function (type, callback) {
        var _this = this;
        ...
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.endpoints.v2EndpointsGet(type, resultsFn);
        }, function (data, done) {
            var devices = data.map(function (device) {
                return connectedDeviceAdapter_1.ConnectedDeviceAdapter.map(device, _this);
            });
            done(null, devices);
        }, callback);
    };
    ...
    ConnectApi.prototype.deleteResource = function (deviceId, path, noResponse, callback) {
        var _this = this;
        ...
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.resources.v2EndpointsDeviceIdResourcePathDelete(deviceId, path, noResponse, resultsFn);
        }, function (data, done) {
            done(null, data[_this.ASYNC_KEY]);
        }, callback);
    };
    ConnectApi.prototype.getResourceValue = function (deviceId, path, cacheOnly, noResponse, callback) {
        var _this = this;
        if (typeof noResponse === "function") {
            callback = noResponse;
            noResponse = false;
        ...
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.resources.v2EndpointsDeviceIdResourcePathGet(deviceId, path, cacheOnly, noResponse, resultsFn);
        }, function (data, done) {
    ...
    ConnectApi.prototype.setResourceValue = function (deviceId, path, value, noResponse, callback) {
        var _this = this;
        if (typeof noResponse === "function") {
            callback = noResponse;
            noResponse = false;
        ...
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.resources.v2EndpointsDeviceIdResourcePathPut(deviceId, path.substr(1), value, noResponse, resultsFn);
        }, function (data, done) {
            var asyncID = data[_this.ASYNC_KEY];
            if (_this.handleNotifications && asyncID) {
                _this._asyncFns[asyncID] = done;
    ...
    ConnectApi.prototype.executeResource = function (deviceId, path, functionName, noResponse, callback) {
        var _this = this;
        if (typeof noResponse === "function") {
            callback = noResponse;
            noResponse = false;
        ...
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.resources.v2EndpointsDeviceIdResourcePathPost(deviceId, path, functionName, noResponse, resultsFn);
        }, function (data, done) {
            var asyncID = data[_this.ASYNC_KEY];
            if (_this.handleNotifications && asyncID) {
                _this._asyncFns[asyncID] = done;
    ...
    ConnectApi.prototype.getResourceSubscription = function (deviceId, path, callback) {
        var _this = this;
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.subscriptions.v2SubscriptionsDeviceIdResourcePathGet(deviceId, path, resultsFn);
        }, function (data, done) {
            done(null, data);
        }, callback);
    };
    ConnectApi.prototype.addResourceSubscription = function (deviceId, path, callback, notifyFn) {
        var _this = this;
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.subscriptions.v2SubscriptionsDeviceIdResourcePathPut(deviceId, path, resultsFn);
        }, function (data, done) {
            if (notifyFn) {
                // Record the function at this path for notifications
                _this._notifyFns[deviceId + path] = notifyFn;
            }
            var asyncID = data[_this.ASYNC_KEY];
            if (_this.handleNotifications && asyncID) {
                _this._asyncFns[asyncID] = done;
    ...
    ConnectApi.prototype.deleteResourceSubscription = function (deviceId, path, callback) {
        var _this = this;
        return functions_1.apiWrapper(function (resultsFn) {
            _this._endpoints.subscriptions.v2SubscriptionsDeviceIdResourcePathDelete(deviceId, path, resultsFn);
        }, function (data, done) {
            // no-one is listening :(
            delete _this._notifyFns[deviceId + path];
            var asyncID = data[_this.ASYNC_KEY];
            if (_this.handleNotifications && asyncID) {
                _this._asyncFns[asyncID] = done;
                return;
    ...
    ConnectApi.prototype.getAccountMetrics = function (options, callback) {
        var _this = this;
        return functions_1.apiWrapper(function (resultsFn) {
            function isPeriod(options) {
                return options.period !== undefined;
            }
            var include = metricAdapter_1.MetricAdapter.mapIncludes(options.include);
            var interval = metricAdapter_1.MetricAdapter.mapTimePeriod(options.interval);
            var start = null;
            var end = null;
            var period = null;
            if (isPeriod(options)) {
            ...
            _this._endpoints.account.v3MetricsGet(include, interval, "", start, end, period, resultsFn);
        }, function (data, done) {
            var list;
    ...
    ConnectApi.prototype.getMetrics = function (options, callback) {
        var _this = this;
        return functions_1.apiWrapper(function (resultsFn) {
            function isPeriod(options) {
                return options.period !== undefined;
            ...
            _this._endpoints.statistics.v3MetricsGet(include, interval, "", start, end, period, resultsFn);
        }, function (data, done) {
            var list;
            if (data.data && data.data.length) {
                list = data.data.map(function (metric) {
